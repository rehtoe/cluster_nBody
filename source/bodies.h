...
struct Particle{
    ...
    Particle(): x(0.0f), y(0.0f), vx(0.0f), vy(0.0f), mass(0.0f), red(255), blue(255), green(255), visualRadius(0.0f) {};
...
class ParticleSimulation{
    ...
    */
    ...
    float optimizeClusters_kmeans(std::map<int, Cluster>& clusterMap);
    void optimizeClusters_FKM(std::map<int, Cluster>& clusterMap);
    void calculateClusterMass(std::map<int, Cluster>& clusterMap);

    float objective_kmeans(std::map<int, Cluster>& clusterMap);
    ...
    int getNextID();
    void pythonPlot(int next_frame_id); 
